/*
  # Fix Edit Sale - Use Movement Amount

  1. Changes
    - Use the account movement's amount instead of transaction amount when updating balances
    - This fixes the balance calculation when editing sales multiple times
*/
...
BEGIN
  ...
  IF v_old_payment_method IN ('Cuenta Corriente', 'cuenta_corriente') AND v_old_client_id IS NOT NULL THEN
    -- Find account movement and get its amount
    SELECT id, amount INTO v_account_movement_id, v_old_movement_amount
    ...
    IF FOUND THEN
      ...
      -- Update old client balance using the movement amount (not transaction amount)
      UPDATE clients
      SET balance = balance - v_old_movement_amount
