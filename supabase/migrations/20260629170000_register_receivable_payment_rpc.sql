/*
  Register accounts receivable payments atomically.

  Keeps pagos, clientes.saldo and ventas.saldo_pendiente in one database
  transaction instead of splitting the workflow across several frontend writes.
*/
...
BEGIN
  ...
  IF p_monto > current_balance THEN
    RAISE EXCEPTION 'El monto no puede ser mayor que el saldo pendiente';
  END IF;

  next_balance := GREATEST(0, current_balance - p_monto);
...
REVOKE ALL ON FUNCTION public.register_receivable_payment(bigint, numeric, text, numeric, numeric, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.register_receivable_payment(bigint, numeric, text, numeric, numeric, date) TO authenticated;
