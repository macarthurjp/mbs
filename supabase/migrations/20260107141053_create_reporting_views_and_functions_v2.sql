/*
  # Reporting System - Views and Functions
  
  1. New Functions
    - `get_sales_by_period` - Returns sales data aggregated by date range
    - `get_top_products` - Returns best selling products with metrics
    - `get_inventory_analysis` - Returns inventory status and valuation
    - `get_current_accounts_status` - Returns account receivable analysis
    - `get_business_profitability` - Returns profitability metrics
  
  2. Views Created
    - `sales_summary_view` - Detailed sales information
    - `product_performance_view` - Product sales metrics
...
WHERE t.type = 'income' AND t.category = 'sale'
...
LEFT JOIN transactions t ON ti.transaction_id = t.id AND t.type = 'income' AND t.category = 'sale'
...
-- Function: Get Inventory Analysis
...
-- Function: Get Current Accounts Status
...
-- Function: Get Business Profitability
