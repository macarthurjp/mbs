/*
  # Fix Credit Card Payment Method V2
  
  1. Changes
    - Update cashbox functions to use only 'Tarjeta' (not 'Tarjeta de Crédito')
    - The correct payment_method in the database is 'Tarjeta'
*/
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(
      CASE 
        WHEN t.type = 'income' AND t.payment_method IN ('Efectivo', 'Transferencia', 'Tarjeta') 
        THEN t.amount 
        ELSE 0 
      END
