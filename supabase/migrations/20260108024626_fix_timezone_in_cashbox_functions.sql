/*
  # Fix Timezone Handling in Cashbox Functions
  
  1. Changes
    - Update `get_cashbox_by_day` to use Argentina timezone
    - Update `get_cashbox_by_period` to use Argentina timezone
    - Update `get_cashbox_by_month` to use Argentina timezone
    - Update `get_cashbox_by_year` to use Argentina timezone
  
  2. Notes
    - All date comparisons and extractions now use Argentina timezone
    - Ensures cashbox reports show correct data for Argentina dates
*/

-- Function: Get Cashbox by Day (Fixed for Argentina timezone)
...
BEGIN
  ...
  FROM transactions
  WHERE (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE < target_date;
  ...
  FROM transactions t
  WHERE (t.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE = target_date;
END;
$$;

-- Function: Get Cashbox by Period (Fixed for Argentina timezone)
...
BEGIN
  ...
  FROM transactions
  WHERE (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE < start_date;
...
-- Function: Get Cashbox by Month (Fixed for Argentina timezone)
...
BEGIN
  ...
  FROM transactions t
  WHERE EXTRACT(YEAR FROM (t.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Argentina/Buenos_Aires')) = target_year
    AND EXTRACT(MONTH FROM (t.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Argentina/Buenos_Aires')) = target_month;
END;
$$;

-- Function: Get Cashbox by Year (Fixed for Argentina timezone)
...
BEGIN
  ...
  SELECT COUNT(DISTINCT EXTRACT(MONTH FROM (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Argentina/Buenos_Aires')))::INTEGER INTO months_count
  FROM transactions
  WHERE EXTRACT(YEAR FROM (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Argentina/Buenos_Aires')) = target_year;
