...
async function getBusinessEmailSettings(req: Request, userId: string, negocioId: string) {
  const supabaseUrl = getEnv('APP_SUPABASE_URL', 'SUPABASE_URL');
  ...
  const defaultFrom = Deno.env.get('RESEND_FROM_EMAIL') || `MatMax Business Suite <facturas@${domain}>`;
...
serve(async (req) => {
  ...
  try {
    ...
    const negocioId = cleanText(venta.negocio_id || body.negocio_id || body.negocioId || body.negocio?.id);
    if (!negocioId) throw new Error('ID del negocio es obligatorio');
    const { business: negocio, from, replyTo } = await getBusinessEmailSettings(req, user.id, negocioId);
    ...
    const invoiceNumber = cleanText(venta.id).slice(0, 8).toUpperCase() || 'INVOICE';
    const currency = cleanText(negocio.moneda) || '€';
    ...
    return jsonResponse(corsHeaders, { success: true, id: emailData?.id || null, invoiceUrl: '' });
  } catch (error) {
