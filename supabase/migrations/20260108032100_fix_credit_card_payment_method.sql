/*
  ...
  1. Changes
    - Update cashbox functions to recognize both 'Tarjeta' and 'Tarjeta de Crédito'
    - This fixes the mismatch between displayed credit card sales and total income
*/
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(
      CASE 
        WHEN t.type = 'income' AND t.payment_method IN ('Efectivo', 'Transferencia', 'Tarjeta', 'Tarjeta de Crédito') 
        THEN t.amount 
        ELSE 0 
      END
    ...
    COALESCE(SUM(
      CASE 
        WHEN t.type = 'income' 
          AND t.category = 'Venta General' 
          AND t.payment_method IN ('Tarjeta', 'Tarjeta de Crédito')
        THEN t.amount 
        ELSE 0 
      END
