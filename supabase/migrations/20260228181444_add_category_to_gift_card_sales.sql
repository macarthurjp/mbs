/*
  # Add category to gift card sales transactions

  1. Problem
    - When a gift card is sold, the transaction doesn't have a category assigned
    - This causes the transaction to not appear in the cashbox dashboard summary
    - The frontend filters by category to calculate totals

  2. Solution
    - Update create_gift_card function to assign category = 'Gift Cards'
    - This will make gift card sales appear in the cashbox totals

  3. Changes
    - Drop and recreate create_gift_card function with category field
*/

-- Drop existing function
DROP FUNCTION IF EXISTS create_gift_card(decimal, text, text, text, uuid, text);

-- Recreate with category field
...
BEGIN
  ...
  -- Create sale transaction for the gift card with category
