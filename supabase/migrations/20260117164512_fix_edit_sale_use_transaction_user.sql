/*
  # Fix Edit Sale - Use Transaction User ID

  1. Changes
    - Use the original transaction's user_id instead of auth.uid()
    - This fixes the null user_id error when creating account movements
*/
...
BEGIN
  ...
  IF p_new_payment_method IN ('Cuenta Corriente', 'cuenta_corriente') AND p_new_client_id IS NOT NULL THEN
    -- Create new account movement using the transaction's user_id
