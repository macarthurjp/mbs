/*
  # Fix Timezone Conversion in Reporting Functions (Corrected)
  
  1. Changes
    - Fix timezone conversion to use correct PostgreSQL syntax
    - Use `AT TIME ZONE 'America/Argentina/Buenos_Aires'` directly
    - Removes incorrect double conversion that was adding a day
  
  2. Functions Updated
    - get_sales_by_period
    - get_top_products
    - get_business_profitability
*/

-- Function: Get Sales by Period (Correct timezone conversion)
...
BEGIN
  RETURN QUERY
  SELECT 
    (t.created_at AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE as sale_date,
  ...
  WHERE (t.created_at AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE BETWEEN start_date AND end_date
    AND t.type = 'income' 
    AND t.category = 'sale'
  GROUP BY (t.created_at AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE
  ORDER BY (t.created_at AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE;
END;
$$;

-- Function: Get Top Products (Correct timezone conversion)
...
BEGIN
  ...
  WHERE t.type = 'income' 
    AND t.category = 'sale'
    AND (start_date IS NULL OR (t.created_at AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE >= start_date)
    AND (end_date IS NULL OR (t.created_at AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE <= end_date)
...
-- Function: Get Business Profitability (Correct timezone conversion)
...
BEGIN
  ...
  SELECT 
    ...
    (SELECT COALESCE(SUM(amount), 0)
     FROM transactions 
     WHERE type = 'income' 
       AND (created_at AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE BETWEEN start_date AND end_date)::NUMERIC as total_cash_in,
    (SELECT COALESCE(SUM(amount), 0)
     FROM transactions 
     WHERE type = 'expense' 
       AND (created_at AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE BETWEEN start_date AND end_date)::NUMERIC as total_cash_out,
    (SELECT COALESCE(
       SUM(CASE WHEN type = 'income' THEN amount ELSE -amount END), 0)
     FROM transactions 
     WHERE (created_at AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE BETWEEN start_date AND end_date)::NUMERIC as net_cash_flow,
  ...
  WHERE (sv.sale_date AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE BETWEEN start_date AND end_date;
END;
$$;